...
function html(opts: remarkWikiLinkOptions) {
  ...
  function exitWikiLink() {
    ...
    let permalink = pagePermalinks.find((p) => permalinkExists(p))
    ...
    if (wikiLinkTransclusion) {
      if (!transclusionFormat[0]) {
        ...
        this.tag(
          `<embed width="100%" data="${hrefTemplate(permalink)}" class="${classNames}" type="application/pdf"/>`
        )
      } else {
        this.tag(
          `<img src="${hrefTemplate(permalink)}" alt="${displayName}" class="${classNames}" />`
        )
      }
    } else {
